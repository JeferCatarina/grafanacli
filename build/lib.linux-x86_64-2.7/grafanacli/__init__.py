...
class GrafanaAdmin(object):
    ...
    def DashboardStar(self, DashboardId):
        run = self.Session.post('{0}/api/user/stars/dashboard/{1}'.format(self.url, DashboardId))
        return (run.json())

    def DashboardUnstar(self, DashboardId):
        run = self.Session.delete('{0}/api/user/stars/dashboard/{1}'.format(self.url, DashboardId))
        return (run.json())
